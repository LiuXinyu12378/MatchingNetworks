...
class g_embedding_bidirectionalLSTM:        #双向lstm神经网络，每一个时间步都进行输出
...
class f_embedding_bidirectionalLSTM:     #一个单向的LSTM单元
    ...
    def __call__(self, support_set, input_image, name, training=False):
        ...
        with tf.name_scope('distance-module' + name), tf.variable_scope('distance-module', reuse=self.reuse):
            ...
            for support_image in tf.unstack(support_set, axis=0):
                sum_support = tf.reduce_sum(tf.square(support_image), 1, keep_dims=True)     #平方和
                support_magnitude = tf.rsqrt(tf.clip_by_value(sum_support, eps, float("inf")))    #平方根倒数计算  分母的倒数
                dot_product = tf.matmul(tf.expand_dims(input_image, 1), tf.expand_dims(support_image, 2))   #分子
                dot_product = tf.squeeze(dot_product, [1, ])
                cosine_similarity = dot_product * support_magnitude    #余弦距离
...
class AttentionalClassify:     #通过得到的余弦距离分数，把这个分数作为attention得分，再乘上标签值support_set_y
    def __init__(self):
        self.reuse = False
...
class MatchingNetwork:     #总的匹配网络
    def __init__(self, support_set_images, support_set_labels, target_image, target_label, dropout_rate,
        ...
        :param support_set_images: A tensor containing the support set images [batch_size, sequence_size, 28, 28, 1]     目标图像 sequence_size应该是类别数
        :param support_set_labels: A tensor containing the support set labels [batch_size, sequence_size, 1]          目标图像标签
        :param target_image: A tensor containing the target image (image to produce label for) [batch_size, 28, 28, 1]    当前要比对的图像
        :param target_label: A tensor containing the target label [batch_size, 1]                                  当前要比对的图像标签
